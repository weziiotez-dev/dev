...
-- ==========================================
-- ระบบล้างแคชและขยะ (Garbage Collection) [Optimized]
-- ==========================================
task.spawn(function()
...
local function fireInGameButton(targetButton)
    if not targetButton or not targetButton.Visible or targetButton.AbsolutePosition.X == 0 then return false end
    ...
    vim:SendMouseButtonEvent(centerX, centerY, 0, true, targetButton, 1)
    task.wait(0.1)
    vim:SendMouseButtonEvent(centerX, centerY, 0, false, targetButton, 1)
    return true
end
...
local function walkToTarget(targetPos, locationName)
    ...
    if success and path.Status == Enum.PathStatus.Success then
        ...
        for i = 2, #waypoints do
            ...
            if (Vector3.new(root.Position.X, 0, root.Position.Z) - Vector3.new(targetPos.X, 0, targetPos.Z)).Magnitude < 4 then break end
            ...
            local timeStuck = tick()
            local isStuck = false
            
            while autoFarmEnabled and isSellingProcess == expectedSellState do
                local currentPos = root.Position
                local distToWp = (Vector3.new(currentPos.X, 0, currentPos.Z) - Vector3.new(wp.Position.X, 0, wp.Position.Z)).Magnitude
                if distToWp < 4 then break end
                
                if wp.Position.Y - currentPos.Y > 2 then hum.Jump = true end
                ...
                if tick() - timeStuck > 1.5 then hum.Jump = true end
                if tick() - timeStuck > 3.0 then isStuck = true break end
...
local function forceFishClick(element)
    ...
    pcall(function()
        ...
        if not button:IsA("GuiButton") and button.Parent and button.Parent:IsA("GuiButton") then button = button.Parent end
        if getconnections and button:IsA("GuiButton") then
            for _, conn in ipairs(getconnections(button.MouseButton1Click)) do pcall(function() conn:Fire() end) pcall(function() conn.Function() end) end
            for _, conn in ipairs(getconnections(button.Activated)) do pcall(function() conn:Fire() end) pcall(function() conn.Function() end) end
            for _, conn in ipairs(getconnections(button.TouchTap)) do pcall(function() conn:Fire() end) pcall(function() conn.Function() end) end
        end
    end)
    ...
    pcall(function()
        ...
        if absSize.X > 0 and absSize.Y > 0 then
            local inset, _ = guiService:GetGuiInset()
            local clickX, clickY = absPos.X + (absSize.X / 2), absPos.Y + (absSize.Y / 2) + inset.Y
            
            pcall(function() vim:SendMouseButtonEvent(clickX, clickY, 0, true, game, 1) end)
            pcall(function() vim:SendTouchEvent(2, 0, clickX, clickY) end) 
            pcall(function() VirtualUser:Button1Down(Vector2.new(clickX, clickY)) end)
            
            task.wait(0.05)
            
            pcall(function() vim:SendMouseButtonEvent(clickX, clickY, 0, false, game, 1) end)
            pcall(function() vim:SendTouchEvent(2, 2, clickX, clickY) end)
            pcall(function() VirtualUser:Button1Up(Vector2.new(clickX, clickY)) end)
        end
    end)
end

local function sendTouchClick(element)
    if not element then return end
    pcall(function()
        local absPos, absSize = element.AbsolutePosition, element.AbsoluteSize
        if absSize.X > 0 and absSize.Y > 0 then
            ...
            local touchY = absPos.Y + (absSize.Y / 2) + inset.Y

            pcall(function() vim:SendTouchEvent(2, 0, touchX, touchY) end)
            task.wait(0.05)
            pcall(function() vim:SendTouchEvent(2, 1, touchX, touchY) end)
            task.wait(0.1)
            pcall(function() vim:SendTouchEvent(2, 2, touchX, touchY) end)
...
local function clickOnce()
    ...
    task.spawn(function()
        pcall(function()
            -- [แก้ไข] เปลี่ยนจุดแตะหน้าจอมินิเกมเป็น "กึ่งกลางหน้าจอ" เป๊ะๆ เพื่อหลบปุ่มเดิน
            local safeX, safeY = camera.ViewportSize.X * 0.5, camera.ViewportSize.Y * 0.5
            pcall(function() vim:SendTouchEvent(1, 0, safeX, safeY) end)
            task.wait(0.05) 
            pcall(function() vim:SendTouchEvent(1, 2, safeX, safeY) end)
            
            -- เผื่อสำหรับคนเล่นบน PC/Emu
                        ...
                        if child3:IsA("ImageButton") then
                            for _, child4 in ipairs(child3:GetChildren()) do
                                if child4:IsA("ImageLabel") then
                                    cachedExtraBtn = child3 return cachedExtraBtn
                                end
                            end
                        end
                    end
    ...
    for _, element in ipairs(mainUI:GetDescendants()) do
        if element:IsA("TextLabel") and element.Text == "Fish" then
            ...
            if parentBtn and (parentBtn:IsA("ImageButton") or parentBtn:IsA("TextButton") or parentBtn:IsA("GuiButton")) then
                cachedFishBtn = parentBtn return cachedFishBtn
            end
        end
    end
    return nil
...
local function getExactMinigameElements()
    ...
    if tick() - lastMinigameScanTime < 0.5 then return nil, nil end
    ...
    for _, container in ipairs(mainUI:GetDescendants()) do
        if container:IsA("ImageLabel") and container.Visible then
            local p3 = container.Parent
            if p3 and p3.Name == "ImageLabel" and p3.Visible then
                local p4 = p3.Parent
                if p4 and p4.Name == "ImageLabel" and p4.Visible then
                    local p5 = p4.Parent
                    if p5 and p5.Name == "ImageLabel" and p5.Visible then
                        local root = p5.Parent
                        if root and root.Name == "MainInterface" then
                            local validChildren = {}
                            for _, child in ipairs(container:GetChildren()) do
                                if child:IsA("ImageLabel") and child.Visible then table.insert(validChildren, child) end
                            end
...
-- ==========================================
-- ระบบคำนวณขนาดหน้าจออัตโนมัติ (Responsive UI)
-- ==========================================
...
local Tabs = {
    ...
    Monitor = Window:AddTab({ Title = "System Cache", Icon = "server" }), -- [แก้ไข] เพิ่ม Tab Monitor
...
AutoFishToggle:OnChanged(function(Value)
    ...
    if not Value then
        ...
        if type(ClearFishingCache) == "function" then
            ClearFishingCache()
        else
...
-- ==========================================
-- [แก้ไข] เพิ่มข้อมูลหน้าต่าง System Monitor (Cache)
-- ==========================================
...
local SysFishLabel = Tabs.Monitor:AddParagraph({ Title = "Fishing Debug", Content = "Step: 0" })
...
-- ==========================================
-- [Optimized] Workspace.ChildAdded
-- ==========================================
...
-- ==========================================
-- [Optimized] Loop อัปเดตข้อมูลตัวละครและ Dashboard (ทุกๆ 15 วินาที)
-- ==========================================
task.spawn(function()
        ...
        pcall(function()
            ...
            if btnCacheMissing then
                ...
                for _, obj in pairs(playerGui:GetDescendants()) do
                    if obj:IsA("GuiButton") and obj.AbsoluteSize.X > 0 and obj.Visible then
                ...
                if not isUIOpen then
                    ...
                    if multiCraftState == "SELECT" then
                        ...
                        if targetBtn and targetBtn.Parent and targetBtn.AbsoluteSize.Y > 0 then
                            ...
                            if fireInGameButton(targetBtn) then
                    ...
                    elseif multiCraftState == "OPEN_RECIPE" then
                        ...
                        if realBtns.open and realBtns.open.Visible then
                            fireInGameButton(realBtns.open)
...
-- ==========================================
-- 🐛 ระบบ Debug UI (กรอบสีเช็คการตรวจจับ)
-- ==========================================
...
-- ==========================================
-- Fishing Background Loops (Walk/Sell/Heartbeat)
-- ==========================================
task.spawn(function()
                ...
                pcall(function()
                    ...
                    if mainUI then
                        ...
                        if sideButtons then
                            ...
                            if btn1 then sendTouchClick(btn1) task.wait(1.5) end
                        end
                    end
                    ...
                    if btn2 then sendTouchClick(btn2) task.wait(1) end
                end)
                    ...
                    if dialog and dialog.Visible then
                        ...
                        if choices and choices.Visible then
                                        ...
                                        if #items > 0 then
                                            ...
                                            if sellAllBtn then 
                                                ...
                                                if targetConfirmBtn then 
                                                    task.wait(1.0)
                                                    sendTouchClick(targetConfirmBtn) 
                                                    task.wait(2.0)
                                                    isConfirmed = true
                                                end
                            ...
                            elseif #validChoices > 0 then
                                sendTouchClick(validChoices[#validChoices])
                                hasCompletedSell = true
								totalSellCount = totalSellCount + 1
                                task.wait(1.5)
                            end
                        else
                            sendTouchClick(dialog)
                            task.wait(0.5)
                        end
                    end
                end
...
RunService.Heartbeat:Connect(function(deltaTime)
    if not autoFarmEnabled or not isAtTarget or isSellingProcess or not DetectMinigame_ON then
        ...
        if safeZoneBar and safeZoneBar.Visible then
            ...
            if diamondIcon and diamondIcon.Visible then
                ...
                elseif distance > FAR_THRESHOLD then
                    ...
                    if currentTime - lastClickTime > 0.005 then
                        clickOnce() 
...
-- ==========================================
-- ระบบ Auto Fish: จัดการรอบตกปลาและ Recovery (30 วิ) พร้อมระบบ Delay 1.5 วิ
-- ==========================================
local fishingRoundCount = 0
local lastFishingStepTime = tick()
local isRecoveryMode = false
local actionFirstDetected = 0 -- ตัวจับเวลาหน้าต่าง Action
...
task.spawn(function()
    while task.wait(0.2) do
        if not autoFarmEnabled or not isAtTarget or isSellingProcess then 
            fishingStep = 0
            hasMinigameMoved = false
        ...
        -- 🚨 Recovery Mode (30 วินาที) 🚨
        if tick() - lastFishingStepTime > 30 and not isRecoveryMode then
            isRecoveryMode = true
        ...
        pcall(function()
            ...
            elseif fishingStep == 2 then
                if not DetectMinigame_ON or not isMinigameActive then
                    fishingStep = 3 
                    lastFishingStepTime = tick()
                    actionFirstDetected = 0 -- รีเซ็ตเวลาเพื่อเตรียมจับเวลารอ Action
                end
            ...
            elseif fishingStep == 3 then
                ...
                if DetectAction_ON and isActionVisible then
                    -- เริ่มจับเวลาเมื่อหน้าต่าง Action เด้งครั้งแรก
                    if actionFirstDetected == 0 then
                        actionFirstDetected = tick()
                    end
                    
                    -- หน่วงเวลา 1.5 วินาทีรอ Animation เล่นเสร็จ
                    if tick() - actionFirstDetected >= 1.5 then
                        ...
                        if FishStatusLabel then FishStatusLabel:SetDesc("⏳ Waiting for Animation... (1.5s)") end
                    end
                else
                    -- ถ้าจู่ๆ หน้าต่างหายไป ให้เริ่มนับเวลาใหม่ในรอบหน้า
                    actionFirstDetected = 0 
